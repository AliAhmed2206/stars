...
import { ArrowLeft, Users, Plus, LogOut, Loader2, Check, Trophy, RefreshCw, Sparkles, Award, Edit3, X, Star, Crown } from "lucide-react"
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 relative z-10">
        ...
        {isCompleted && champion && (
          <div className="glass rounded-3xl p-8 mb-8 text-center animate-slide-up border border-success/20 relative overflow-hidden">
            ...
            <div className="relative">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-success/10 text-success text-sm mb-4 animate-pulse-glow">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-3 items-center">
                <span className={cn(
                  ...
                  tournament.status === "completed" && "border-success/30 text-success animate-breathe",
                )}>
            ...
            <div className="flex gap-2 flex-wrap">
              {user && !isParticipant && tournament.status === "open" && (
                <button onClick={handleJoin} className="btn-primary px-5 py-2.5 rounded-2xl text-sm flex items-center gap-2"><Plus className="w-4 h-4" /> Join</button>
              )}
              {user && isParticipant && tournament.status === "open" && (
                <button onClick={handleLeave} className="btn-secondary px-5 py-2.5 rounded-2xl text-sm flex items-center gap-2 text-danger"><LogOut className="w-4 h-4" /> Leave</button>
              )}
              {canStart && (
                <button onClick={handleGenerateBracket} className="btn-primary px-5 py-2.5 rounded-2xl text-sm flex items-center gap-2"><RefreshCw className="w-4 h-4" /> Start Tournament</button>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-1 space-y-4">
            <div className="glass rounded-2xl p-5 animate-slide-in-left">
              ...
              {participants.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {participants.map((p, i) => {
                    const isWinner = champion?.id === p.user_id
                    return (
                      <div key={p.id} className={cn(
                        "flex items-center gap-2.5 glass rounded-xl px-3 py-2 transition-all",
                        isWinner && "border border-yellow-500/30 bg-yellow-500/5"
                      )}>
                        <div className={cn(
                          "w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0",
                          isWinner ? "bg-yellow-500 text-black" : "bg-gradient-to-br from-accent to-accent2 text-white"
                        )}>
                          {isWinner ? <Crown className="w-3 h-3" /> : i + 1}
                        </div>
                        <span className={cn("text-sm truncate", isWinner && "text-yellow-400 font-bold")}>
                          {p.profile?.username || "Anonymous"}
                        </span>
                        {isWinner && <Sparkles className="w-3 h-3 text-yellow-400 flex-shrink-0 animate-sparkle" />}
                      </div>
                    )
                  })}
                </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            {matches.length === 0 ? (
              <div className="glass rounded-2xl p-12 text-center animate-slide-up">
                ...
                {canStart && (
                  <button onClick={handleGenerateBracket} className="btn-primary px-8 py-3 rounded-2xl text-base inline-flex items-center gap-2 animate-glow">
                    ...
                    <div key={round} className="bracket-round">
                      <p className="text-xs text-muted font-medium mb-3 text-center uppercase tracking-wider">
                        ...
                        {round === maxRound && isCompleted && (
                          <span className="block text-success text-xs mt-1 animate-pulse-glow">🏆 Champion</span>
                        )}
                      </p>
                      {roundMatches.map((match, idx) => (
                        <div key={match.id} className={cn(
                          "bracket-match",
                          round === maxRound && "font-bold"
                        )}>
                          <MatchCard
                            match={match}
                            user={user}
